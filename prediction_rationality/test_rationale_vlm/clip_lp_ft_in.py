...
def main(args):
    ...
    model = LP_Net(clip_model, 512, 1000, freeze_clip=False).to(device)
    ...
    for iter, labeled_batch in enumerate(in_val_dl):
        ...
        for si in range(cur_batch_size):
            vis, img_f, xai_map_seg, pred_idx = generate_visualization_IN(
            ...
            rma = eval_rma(pred=xai_map_seg, anno=anno_mask, cls_thres=0.5)
            ES_list.append(rma)
